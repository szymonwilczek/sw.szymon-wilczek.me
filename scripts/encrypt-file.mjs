...
const ciphertext = Buffer.concat([cipher.update(rawData), cipher.final(), cipher.getAuthTag()]);
...
const formattedSize = rawData.length > 1024 * 1024 ? `${(rawData.length / (1024 * 1024)).toFixed(1)} MB` : `${sizeKb} KB`;

const jsonSlug = filename.replace(/[^a-zA-Z0-9_-]/g, '-').toLowerCase();
const jsonPath = path.join(process.cwd(), 'src', 'content', 'files', `${jsonSlug}.json`);
