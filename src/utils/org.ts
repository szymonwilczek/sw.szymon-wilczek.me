...
const efAutumnTheme = {
	...
	tokenColors: [
		...
		{
			scope: ['keyword', 'keyword.control', 'keyword.operator.word', 'keyword.other', 'storage', 'storage.type'],
			settings: {
				foreground: '#d0730f',
				fontStyle: 'bold',
			},
		...
		visit(tree, 'element', (node: any, index: number | undefined, parent: any) => {
			if (node.tagName === 'pre') {
				...
				if (codeNode) {
					...
					const langClass = classList.find((c: string) => typeof c === 'string' && c.startsWith('language-'));
		...
		visit(tree, 'element', (node: any, index: number | undefined, parent: any) => {
			...
			if (
				...
				(node.properties.className.includes('example') ||
					(Array.isArray(node.properties.className) && node.properties.className.some((c: string) => c.includes('example'))))
			) {
				node.tagName = 'pre';
			...
			if (/^h[1-6]$/.test(node.tagName)) {
				if (node.children && node.children.length > 0) {
					for (const child of node.children) {
						if (child.properties?.className) {
							const classes = Array.isArray(child.properties.className)
								...
								: [child.properties.className];
							if (classes.some((c: string) => c.includes('todo-keyword') || c === 'TODO' || c === 'DONE')) {
					...
					if (first.type === 'text') {
						const match = first.value.match(/^(WAITING|NEXT|HOLD|CANCELLED|TODO|DONE)\s+(\[#[A-Z]\]\s+)?(.*)$/);
						if (match) {
							const state = match[1];
						...
						if (cbMatch) {
							...
							const checkboxNode: any = {
								...
								properties: {
									type: 'checkbox',
									disabled: true,
									className: ['org-checkbox', isIndeterminate ? 'is-indeterminate' : ''].filter(Boolean),
								},
								children: [],
							};
...
export async function renderOrg(orgContent: string): Promise<string> {
	...
	const processor = unified().use(uniorgParse).use(uniorgRehype).use(rehypeOrgEnhancements).use(rehypeShikiHighlight, highlighter).use(rehypeStringify);
...
export async function renderContentEntry(entry: any) {
	const isOrg = entry.filePath?.endsWith('.org') || entry.id?.endsWith('.org') || entry.body?.includes('#+title:');
